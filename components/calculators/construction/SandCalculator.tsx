...
export function SandCalculator() {
  ...
  const { results, error, formulaDesc } = useMemo(() => {
    ...
    // HARDCODED: 'sand' density (2,600 lbs/yd³)
    const outputs = calculateGravel(shape, dims, unit, 'sand', compactionPct);

    let formulaStr = '';
    const compText = compactionPct > 0 ? ` × 1.${compactionPct}` : '';
    switch (shape) {
      case 'rectangle':
        formulaStr = `Volume = (Length × Width × Depth)${compText}`;
        break;
      case 'square':
        formulaStr = `Volume = (Side² × Depth)${compText}`;
        break;
      case 'circular':
        formulaStr = `Volume = (π × (Diameter ÷ 2)² × Depth)${compText}`;
        break;
    }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12">
        ...
        <div className="lg:col-span-6 xl:col-span-5 space-y-8">
          ...
          <div className="space-y-6">
            <div className="space-y-3">
              ...
              >
                <option value="rectangle">Rectangle (Sandbox / Paver Base)</option>
                <option value="square">Square</option>
                <option value="circular">Circular (Firepit / Pool Base)</option>
              </select>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              ...
              {(shape === 'rectangle' || shape === 'square') && (
                <div className="space-y-2">
                  ...
                  <NumberInput value={length} onChange={(e) => setLength(e.target.value)} min="0" placeholder="e.g. 8" suffixNode={<span>{unit}</span>} />
                </div>
              )}
              ...
              {shape === 'rectangle' && (
                <div className="space-y-2">
                  <Label>Width</Label>
                  <NumberInput value={width} onChange={(e) => setWidth(e.target.value)} min="0" placeholder="e.g. 8" suffixNode={<span>{unit}</span>} />
                </div>
              )}
              ...
              <div className="space-y-2">
                ...
                <NumberInput value={depth} onChange={(e) => setDepth(e.target.value)} min="0" placeholder="e.g. 6" suffixNode={<span>{unit}</span>} />
              </div>
          ...
          <div className="space-y-3">
            <div className="flex justify-between items-end">
              <Label>Compaction Shrinkage Factor</Label>
              <span className="text-sm font-medium text-amber-600 dark:text-amber-500">+{compactionPct}%</span>
            </div>
            <select
              className="flex h-10 w-full rounded-md border border-border bg-background px-3 py-2 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-500"
              value={compactionPct}
            ...
            >
              <option value={0}>0% (Loose, e.g. Children's Sandbox)</option>
              <option value={5}>5% (Lightly Tamped)</option>
              <option value={10}>10% (Plate Compacted, e.g. Paver Base)</option>
              <option value={15}>15% (Heavy Compression)</option>
            </select>
          </div>
        ...
        <div className="lg:col-span-6 xl:col-span-7 space-y-6">
          ...
          <div className="bg-secondary-50/50 dark:bg-secondary-900/10 p-6 rounded-2xl border border-border">
            <ResultDisplay 
              label="Bulk Sand Required"
              value={!results ? '--' : formatUnit(results.totalTons)}
              subValue="Tons"
              className="mb-6 bg-gradient-to-br from-yellow-50 to-amber-100 dark:from-yellow-950/30 dark:to-amber-950/30 border-amber-200 dark:border-amber-900 shadow-sm"
            />
          ...
          <div className="bg-secondary-100 dark:bg-secondary-800/50 p-4 rounded-xl text-sm text-secondary-600 dark:text-secondary-400">
            ...
            <br/><br/>
            Standard Sand (Mason/Concrete) weighs approximately 2,600 pounds per cubic yard (1.3 Tons). The calculator computes your raw geometry, applies your requested Compaction Factor, and multiplies by 1.3 to determine the exact payload Tonnage.
          </div>
