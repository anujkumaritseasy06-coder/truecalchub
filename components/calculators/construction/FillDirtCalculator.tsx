...
import { 
  ...
  FillDirtDimensions, 
  calculateFillDirt 
} from '@/lib/calculators/fillDirt';

export function FillDirtCalculator() {
  const [shape, setShape] = useState<FillDirtShape>('rectangle');
  const [unit, setUnit] = useState<FillDirtUnit>('feet');
  ...
  // Compaction Factor
  const [compactionPct, setCompactionPct] = useState<number>(20);
  ...
  const { results, error, formulaDesc } = useMemo(() => {
    ...
    const outputs = calculateFillDirt(shape, dims, unit, compactionPct);

    let formulaStr = '';
    const compText = compactionPct > 0 ? ` × 1.${compactionPct}` : '';
    switch (shape) {
      case 'rectangle':
        formulaStr = `Cubic Yards = ((Length × Width × Depth)${compText}) ÷ 27`;
        break;
      case 'circular':
        formulaStr = `Cubic Yards = ((π × (Diameter ÷ 2)² × Depth)${compText}) ÷ 27`;
        break;
    }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12">
        ...
        <div className="lg:col-span-6 xl:col-span-5 space-y-8">
          ...
          <div className="space-y-6">
            <div className="space-y-3">
              ...
              <select
                ...
                value={shape}
                onChange={(e) => {
                  setShape(e.target.value as FillDirtShape);
                  setLength(''); setWidth(''); setDepth(''); setDiameter('');
                }}
              >
                <option value="rectangle">Rectangle (Trench / Foundation)</option>
                <option value="circular">Circular (Sinkhole / Pool Removal)</option>
              </select>
            </div>
            ...
            <div className="space-y-3">
              ...
              <select
                className="flex h-10 w-full rounded-md border border-border bg-background px-3 py-2 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-500"
                value={unit}
                onChange={(e) => setUnit(e.target.value as FillDirtUnit)}
              >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              ...
              {shape === 'circular' && (
                <div className="space-y-2">
                  ...
                  <NumberInput value={diameter} onChange={(e) => setDiameter(e.target.value)} min="0" placeholder="e.g. 15" suffixNode={<span>{unit}</span>} />
                </div>
              )}

              <div className="space-y-2">
                <Label>Depth / Thickness</Label>
                <NumberInput value={depth} onChange={(e) => setDepth(e.target.value)} min="0" placeholder="e.g. 24" suffixNode={<span>{unit}</span>} />
              </div>
          ...
          <div className="space-y-3">
            <div className="flex justify-between items-end">
              <Label>Mechanical Compaction Factor</Label>
            ...
            >
              <option value={10}>10% (Light Tamping / Hand Tamp)</option>
              <option value={15}>15% (Standard Settling)</option>
              <option value={20}>20% (Plate Compactor / Roller)</option>
              <option value={30}>30% (Deep Trench / Heavy Rolling)</option>
            </select>
            <p className="text-xs text-secondary-500 mt-1">Fill dirt must be mechanically compacted in 6-inch "lifts" to prevent future sinkholes.</p>
          </div>
        ...
        <div className="lg:col-span-6 xl:col-span-7 space-y-6">
            ...
            <div className="bg-card p-6 rounded-2xl border border-border flex flex-col justify-between">
              ...
              {results && results.bags40lb > 50 && (
                 <p className="text-xs text-stone-700 dark:text-stone-400 mt-4 leading-relaxed font-bold border-t border-border pt-4">
                   ⚠️ DO NOT BUY BAGS. Your project requires {formatUnit(results.totalTons)} Tons of structural fill. Buying {results.bags40lb} bags of dirt at a hardware store is financially absurd. Call a local quarry for a dump truck delivery.
                 </p>
              )}
            </div>
          ...
          <div className="bg-secondary-100 dark:bg-secondary-800/50 p-4 rounded-xl text-sm text-secondary-600 dark:text-secondary-400">
            ...
            <br/><br/>
            Fill dirt is dense subsoil sold interchangeably by the Yard or the Ton. The landscaping industry standard conversion is exactly <strong>1 Cubic Yard = 2,000 lbs (1 Ton)</strong>. The calculator computes your raw volume, applies your chosen Compaction Factor to ensure you have enough dirt after running heavy machinery over it, and outputs the identical Yardage/Tonnage payloads.
          </div>
