        ...
        <div className="lg:col-span-7 space-y-6">
          ...
          <div className="bg-secondary-50/50 dark:bg-secondary-900/10 p-6 rounded-2xl border border-border">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-sm font-bold text-secondary-500 uppercase tracking-wider">Roof Geometry</h3>
              {results && (
            ...
            <div className="grid grid-cols-1 mb-6">
              <div className="bg-gradient-to-br from-primary-600 to-primary-700 dark:from-primary-900/80 dark:to-primary-800/80 p-6 rounded-xl border border-primary-500/30 shadow-sm text-center flex flex-col items-center justify-center">
                <p className="text-xs text-primary-100 uppercase tracking-wider mb-2 font-bold">Standard Industry Pitch</p>
                <div className="flex items-end space-x-2">
                   <p className="text-5xl sm:text-6xl font-bold text-white tracking-tight">
                     {!results ? '0' : results.pitchX}
                   </p>
                   <p className="text-2xl sm:text-3xl font-medium text-primary-200 mb-1">
                     / 12
                   </p>
                </div>
                <p className="text-sm text-primary-200 mt-3">
                  This roof rises {!results ? '0' : results.pitchX} inches for every 12 inches it runs.
                </p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              <ResultDisplay 
                label="Angle"
                value={!results ? '--' : `${results.angleDegrees}°`}
              ...
              <ResultDisplay 
                ...
                value={!results ? '--' : `${results.slopePercentage}%`}
                className="bg-background border border-border shadow-sm col-span-2 md:col-span-1 p-4"
                valueClassName="text-2xl lg:text-3xl"
              />
            </div>
            
            <div className="mt-6 p-4 bg-sky-50 dark:bg-sky-900/20 rounded-xl border border-sky-100 dark:border-sky-800">
              <h4 className="text-sm font-bold text-sky-800 dark:text-sky-300 mb-2">Why is the Multiplier Important?</h4>
              <p className="text-xs text-sky-700/80 dark:text-sky-400/80 leading-relaxed">
                If you are trying to calculate roofing materials, you must multiply the flat square footage of your attic/house footprint by the <strong>{!results ? '--' : results.pitchMultiplier}x Multiplier</strong> to find the true surface area of the roof. 
              </p>
              <div className="mt-3">
                 <a href="/construction/roofing-calculator" className="text-xs font-bold text-primary-600 hover:text-primary-700 underline">Use the Full Roofing Material Calculator &rarr;</a>
              </div>
            </div>
          </div>
