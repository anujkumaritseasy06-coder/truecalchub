export type FillDirtShape = 'rectangle' | 'circular';
export type FillDirtUnit = 'feet' | 'inches' | 'yards' | 'meters';

export interface FillDirtDimensions {
  length?: number;
  width?: number;
  depth?: number;
  diameter?: number;
...
export interface FillDirtCalculationResult {
  cubicFeet: number;
  cubicYards: number;
  totalTons: number;
  totalPounds: number;
...
export function convertToFeet(value: number, unit: FillDirtUnit): number {
